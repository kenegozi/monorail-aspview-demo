...
function createEl(eltype,options,vl,elm) {
	...
	switch (eltype)
	{
		...
		case "select" :
			...
			if(options.value) {
				var ovm = [];
				if(msl) {jQuery(elem).attr({multiple:"multiple"}); ovm = vl.split(","); ovm = $.map(ovm,function(n){return $.trim(n)});}
				...
				if(typeof options.value == 'string') {
					...
					for(var i=0; i<so.length;i++){
						...
						if (msl && jQuery.inArray($.trim(sv[1]), ovm)>-1) {ov.selected ="selected";}
						elem.appendChild(ov);
					}
					...
					for ( var key in oSv) {
						...
						if (msl && jQuery.inArray($.trim(oSv[key]),ovm)>-1) ov.selected ="selected";
						elem.appendChild(ov);
					}
