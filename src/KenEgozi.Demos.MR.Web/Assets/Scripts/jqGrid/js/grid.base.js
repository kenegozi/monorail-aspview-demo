;(function ($) {
/*
 * jqGrid  3.4 beta2- jQuery Grid
 * Copyright (c) 2008, Tony Tomov, tony@trirand.com
 * Dual licensed under the MIT and GPL licenses
 * http://www.opensource.org/licenses/mit-license.php
 * http://www.gnu.org/licenses/gpl.html
 * Date: 2009-01-11 rev 75
 */
$.fn.jqGrid = function( p ) {
	...
	$.fn.getCell = function(rowid,iCol) {
		...
		this.each(function(){
			...
			if(rowid && iCol>=0) {
				var ind = $($t).getInd($t.rows,rowid);
				if(ind) {
					ret = $.htmlDecode($("td:eq("+iCol+")",$t.rows[ind]).html());
				}
			}
		});
		return ret;
	...
	return this.each( function() {
		...
		var addXmlData = function addXmlData (xml,t, rcnt) {
			if(xml) { var fpos = ts.p.treeANode; rcnt=rcnt ||0; if(fpos===0 && rcnt===0) {$("tbody tr:gt(0)", t).remove();} } else { return; }
		...
		var addJSONData = function(data,t, rcnt) {
			...
			var i,j,row,f=[],cur,gi=0,si=0,drows,idn,rd=[],cn=(ts.p.altRows===true) ? 'alt':'';
		...
		var sortArrayData = function() {
			...
			if (st == 'float' || st== 'number') {
				...
				findSortKey = function($cell) {
					return $.trim($cell.text().toUpperCase());
				};
			}
			var rows=[];
		...
		var sortData = function (index, idxcol,reload){
			...
			ts.p.savedRow =[];
			var scg = ts.p.scroll; if(ts.p.scroll===true) {ts.p.scroll=false;}
			populate();
		...
		if(ts.p.caption) {
			$(grid.cDiv,ts).width(grid.width).css("text-align","center").slideDown(0);
